...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        trait_prop = PROBS['trait'][gene_number][traits]

        if people[person]['mother'] is None:
            # no parents, use probability distribution
            choice *= gene_numb_prop * trait_prop
        else:
            ...
            if gene_number == 0:
                ...
                choice *= (1 - percentages[mother]) * (1 - percentages[father])
            elif gene_number == 1:
                # 1, one of parents gave gene
                choice *= (1 - percentages[mother]) * percentages[father] + percentages[mother] * (1 - percentages[father])
            else:
                # 2, both of parents gave gene
                choice *= percentages[mother] * percentages[father]
...
def update(probabilities, one_gene, two_genes, have_trait, p):
    ...
    for index in probabilities:
        gene_number = 1 if index in one_gene else 2 if index in two_genes else 0
        probabilities[index]["gene"][gene_number] += p
        probabilities[index]["trait"][index in have_trait] += p
...
def normalize(probabilities):
    ...
    for count in probabilities:
        for index in ['gene', 'trait']:
            ...
            for category in probabilities[count][index]:
                val = probabilities[count][index][category]
                normalized_val = val / summed
                normalSet[count][index][category] = normalized_val
    return normalSet
