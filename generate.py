...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        """
        for index, words in self.domains.items():  # Iterate over all variables and their potential words
            words_to_remove = set()  # Set to store words that will be removed from the variable's potential words
            for word in words:
                if len(word) != index.length:  # If word is not of the same length as the variable can hold, delete
                    words_to_remove.add(word)
            # Subtract the words that are unsuitable from variable's domain
            self.domains[index] = words.difference(words_to_remove)
    ...
    def revise(self, x, y):
        ...
        """
        revise = False  # set revised to false to see if any consistency will be set
        ...
        if overlap:
            ...
            if xs_to_remove:  # if there are x variables to remove from the x domain, remove
                ...
                revise = True  # set revised to true to shows that a revision was made
    ...
    def ac3(self, arcs=None):
        ...
        while arcs:
            index, count = arcs.pop()
            if self.revise(index, count):  # revise each combination of nodes in an edge (arc)
                # if there are no variables for x, return False, meaning that arc
                # consistency is impossible
                if len(self.domains[index]) == 0:
                    return False

                # take all x's neighbors except Y and add the edges between them and x to the queue
                for info in self.crossword.neighbors(index) - {count}:
                    arcs.appendleft((info, index))
    ...
    def consistent(self, assignment):
        ...
        for variableX, wordX in assignment.items():
            if variableX.length != len(wordX):  # check if assigned word is of the proper length for the variable
                return False
            ...
            for variableY, wordY in assignment.items():
                if variableX != variableY:
                    ...
                    overlap = self.crossword.overlaps[variableX, variableY]  # it returns indices where they overlap
                    if overlap:  # if there is an overlap, make sure that the certain character is the same
                        a, b = overlap
                        if wordX[a] != wordY[b]:  # if the characters are different, then it is inconsistent
                            return False
