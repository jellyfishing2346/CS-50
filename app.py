...
def index():
    ...
    stockInfo = db.execute(
        "SELECT symbol, SUM(shares) as shares, operation FROM stock WHERE userID = ? GROUP BY symbol HAVING (SUM(shares)) > 0;",
        session["user_id"],
    )
    totalStocks = 0
...
def buy():
    ...
    if request.method == "POST":
        ...
        if cash < (sharePrice):
            ...
            db.execute( "INSERT INTO users (userID, symbol, shares, price, operation) VALUES (?, ?, ?, ?, ?)", session["user_id"], symbol.upper(),shares, price["price"], "buy",
            )
