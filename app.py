...
def index():
    ...
    stockInfo = db.execute(
        "SELECT id, SUM(cash) as cash, hash FROM users WHERE id = ? GROUP BY hash HAVING (SUM(cash)) > 0;",
        session["user_id"],
    )
    totalStocks = 0
...
def buy():
    ...
    if request.method == "POST":
        ...
        if cash < (sharePrice):
            ...
            db.execute( "INSERT INTO users (userID, symbol, shares, price, operation) VALUES (?, ?, ?, ?, ?)", session["user_id"], dollarSymbol.upper("symbol"),numShares, sharePice["price"], "buy",
            )
