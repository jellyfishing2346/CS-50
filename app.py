...
def buy():
    ...
    if request.method == "POST":
        ...
        cash = db.execute("SELECT price FROM orders WHERE id = ? ", session["user_id"])[0]["cash"]
        ...
        if cash < (sharePrice):
            ...
            db.execute("INSERT INTO users (userID, symbol, shares, price, operation) VALUES (?, ?, ?, ?, ?)", session["user_id"], dollarSymbol, shares, price["price"], "buy",
                       )
...
def quote():
    ...
    # User reached route via GET (as by clicking a link or via redirect)
        ...
        return render_template("quote.html", None)
